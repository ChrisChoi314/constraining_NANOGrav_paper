...
\end{equation}
\red{We are interested in $v_k(\tau)$ when it is in the massless regime, for large $\tau$. Solving the EoM (\ref{eqn:eom}), applying the appropriate intial and boundary conditions, leads to the following: \begin{equation} \label{eqn:mode_phase_3}
...
\red{The values for $H_{\inf}$ we find that reproduce the SGWB are very low. However, models of inflation with such a low $H_{\inf}$ do exist. For example, a low-scale supersymmetry model with an inflaton field identified with the Higgs boson was proposed in Ref.\ \cite{Nakayama:2011ri}. Such a model is an extension of the standard model including three right-handed neutrinos and the Higgs boson responsible for breaking the $U(1)_{B-L}$ symmetry. In addition, Ref.\ \cite{Takahashi:2018tdu} proposes a model of inflation with $H_{\inf}$ on the order of $10^{-4}$ to $10^{-1}$ GeV. Such a model requires, however, a more precise treatment of the parameters to achieve reheating, as well as a weaker coupling to standard model particles to avoid excessive flatness of the inflaton potential. Additionally, implications of the non-MG inflationary origin of the SGWB have been explored in Ref.\ \cite{EPTA:2023xxk}, which suggest that non-standard inflationary theories are required if they are to explain the SGWB, including features like breaking the slow-roll consistency relation and having a blue-tilted spectrum, which MTMG exactly provides.}
